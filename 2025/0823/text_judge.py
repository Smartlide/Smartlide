...
def clean_mark_text(text):
    # 把指令關鍵字去掉
    for kw in ["畫底線", "畫重點", "標記重點", "底線", "畫螢光筆", "螢光筆"]:
...
def predict_slide_action(text):
    prompt = (
        ...
        "3. 當「要翻頁的機率」>= 90% 時，輸出 'N'，表示需要往後翻頁（下一頁）。\n"
        "4. 當「要翻頁的機率」< 90% 時，輸出 'S'，表示不需要翻頁。\n"
        ...
        "9. 只有在明確提到頁數時才輸出數字。\n"
        "10. 當語句明確表示『畫底線』、『化底線』『標記重點』、『畫重點』、『化重點』等，請輸出 'U'。\n"
        "11. 當語句明確表示『畫螢光筆』、『螢光筆』、『化螢光筆』，請輸出 'H'。\n"
        "語境判斷指引：\n"
    ...
    while True:
        ...
        # 判斷是否為畫底線指令
        if output.lower() == 'u':
            underline_content = clean_mark_text(text.strip())
            return f"U:{underline_content}"  # 返回標記的關鍵字串

        # 判斷是否為畫螢光筆指令
        elif output.lower() == 'h':
            highlight_content = clean_mark_text(text.strip())
            return f"H:{highlight_content}"  # 返回標記的關鍵字串
